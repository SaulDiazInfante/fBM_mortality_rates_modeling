...
%
   Denote by $h_0$ the initial 
    ...
    asymptotically follows the relation:
    \[
        (R/S)_n \sim c n^H,
    \]
    ...
    plot is roughly a straight line with slope $\num{0.5}$. If the process is 
    persistent, then the slope $H$ is greater than $\num{0.5}$. If the processes is 
    anti-persistent, then the slope $H$ is less than $\num{0.5}$. The "significance" 
...
\subsection{Estimation of $\sigma$ and  $\lambda$}
    ...
    \begin{definition}
        ...
        \]
    \end{definition}
    \todo{Write Definition of consistency}
    \begin{theorem}
...
%
    ...
    \end{figure}
        ...
        \todo{Note that both plots are equal}
        \todo{add a table with the estimation parameters for each age}
