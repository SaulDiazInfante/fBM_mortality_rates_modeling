...
\label{intro}
    ...
    \info{Add relevant use and implications of our results}
        ...
        After this brief introduction, the model we are interested is introduced
    in \Cref{model-form}. \Cref{fgn} reviews the fBM and presents
...
\section{Model formulation}\label{model-form}
    ...
    (see \cite{mi-pr} for the original paper or \cite{gi-or-be}for a recent 
    generalization). 
    ...
    \begin{align}\label{E-surv}
        S(t,T):=
            ...
            \right],
    \end{align}
where filtration $\{\mathcal{F}_t\}_{t\ge 0} $ represent the information 
until time $t$ and $h_x(t)$ denotes stochastic force of mortality or hazard rate.
...
\begin{theorem}
    ...
    \begin{align*}
        \sqrt{N}(\hat{H}_N  - H ) 
            ...
            & 
                \stackrel{\mathcal{L}}{\to} N 
                (0, \Gamma_2 (\bm{a},\sigma,H))
                \quad
                \hbox{\rm as} 
                \quad N 
                \to +\infty,
...
With this result we obtain an estimator for the parameters
$\sigma$.
...
\begin{figure}[H]
<<<<<<< HEAD
    \includegraphics{PlotMenForecast50.eps}
    \includegraphics{PlotMenForecast60.eps}
    \includegraphics{PlotMenForecast70.eps}
    \includegraphics{PlotMenForecast90.eps}
=======
<<<<<<< HEAD
