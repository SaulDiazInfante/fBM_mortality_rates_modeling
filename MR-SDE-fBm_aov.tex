...
\section{Introduction} \label{intro}
    ...
    (fBM) \cite{ma-va}. Since fBM is a generalization of the standard Brownian 
    Motion (BM) that still satisfies self-similarity and is LRD, fBM results to 
    be a natural noise model to describe LRD behavior.
...
\section{Model formulation}\label{model-form}
    ...
    \begin{equation}
        ...
        \end{aligned}
    \end{equation}
    Is well know that this fOU process is Long time depndent when 
    $H \in [1/2,1)$, for details see 
    \citet{Cheredito2003,Anh2002,Hu2005,Kleptsyna2002}. Thus results natural
    explore long time memory effects with this nosie model.
    \begin{align}
        Y_t^H&=-\lambda\int_0^t Y_s^Hds+\sigma B_t^H.\label{mod3}
    \end{align}
    The equation above lacks of a stochastic integral because we
    consider the case with additive noise.  We refer the reader to \cite{mi} 
    for further reading on fractional stochastic calculus.
...
%
    ...
    Gaussian and ergodic.
        ...
        Moreover, when $H \in(1/2,1)$, $Y_t$ even presents the long-range 
    dependence property \cite{ch-ka-ma,ra}.
