...
%
    \institute{
        Francisco Delgado-Vences
        \at
        Conacyt-Universidad Autonomoa de Mexico.
        \\
...
\label{intro}
    ...
    without memory effects.  Pitacco et al. provide a review with a history
    of the first mortality tables.  Milevesky and Promislow report in
    \cite{mi-pr} a linear SDE driven by Brownian motion that describes
    ...
    autoregressive model for the logarithm of the hazard rate.   Jevtić et al.
    \cite{je-lu-vi} formulate a cohort-based model with imperfect correlation
    across generations and use data from 1900-2008 of the UK to estimate
    parameters.

        The present paper aims to show statistical evidence that mortality rates
    could follow a stochastic process with long-range dependence (LRD), that is,
    ...
    below we give a formal definition.
        ...
        We base our stochastic formulation in the canonical example of a
    continuous-time LRD process--the fractional Brownian Motion (fBM). We refer
    to the seminal work of Mandelbrot and van Ness \cite{ma-va} to more
    details. Since fBM is a generalization of the standard Brownian Motion (BM)
    and still satisfies self-similarity and is LRD, fBM results to be a natural
    noise model to describe and to explore LRD behavior.

        Our main idea is to extend the stochastic model reported by Milevesky
     and Promislow to a model with LRD and verify its performance to fitting
    and forecasting with real data.  With this technique, we obtain statistical
    evidence via parameter estimation and forecasting that our LRD model
    captures an unexpectedly long time persistence pattern of Italy mortality
    rate.

        Our contribution suggest...
        \info{Add relevant use and implications of our results}

        According to data of Italy, our LRD model captures essential events in
    mortality rates as the abrupt changes. For example, our formulation
    explains the abrupt growing mortality rate in men between its
    18-50-year-old due to the second war world.
    \improvement{To improve \newline and review information}

        After this brief introduction, \Cref{fgn} reviews the fBM and presents
    the fractional Ornstein-Uhlenbeck (fOU) process. Section 3 outlines the
    applied method for parameter estimation. In  Section 4, we implement our
    ...
    its efficiency. Finally, we conclude in Section 5 with relevant
...
\section{Fractional Brownian motion} \label{fgn}
    ...
    (fBm) and was introduced by Kolmogorov \cite{ko} and studied by Mandelbrot
    ...
    independent increments, also it is a continuous-time Gaussian process.
    ...
    H\"{o}lder continuous of any order strictly less than H, that is, for each
    ...
    \]
        ...
        For $H =\tfrac{1}{2}$ the covariance can be written as
    $
        R_{1 / 2} (t,s)
            = \min(s, t)
    $ and the process $B_t^{1/2}$, is equivalent to
    the standard Brownian motion. However, unlike the standard BM,
    if $H \ne\tfrac{1}{2}$, then  this increments are \emph{not}
    independent.
...
\begin{itemize}
    \item
        ...
        $\sum_{n=1}^\infty \rho_H(n)=\infty$, then
        the process $X_n$ is persistent with positive correlation.
        In this case, we say that $X_n$ has the \emph{ long-range dependence}
        property
    \item
        if $H < \tfrac{1}{2}$, then $\rho_H(n) < 0$ for $n$ large enough and
...
The subsequent sections are devoted to present several methods to estimate the
Hurst parameter for the fBM.
