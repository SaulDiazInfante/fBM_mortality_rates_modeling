...
datos<-drates[drates$Year%in%c(1950:2004) & drates$Age==ages[A+1], ]
  ...
  # for(t in 1:cy)
  # { 
  #   linea<-datos$Male[t]-datos$Male[1]
  #   if(linea<0)
  #     {
  #       cont<-cont+1
  #     }
...
T0Me[A+1]<-t0M*2/3 
  ...
  # for(t in 1:cy)
  # { 
  #   linea<-datos$Female[t]-datos$Female[1]
  #   if(linea<0)
  #   {
  #     cont<-cont+1
  #   }
  ...
  T0Wo[A+1]<-t0W*2/3
   ...
   for(B in years) {
      Rsum1<-Rsum1 +  ((datos$Year[datos$Year==B])-1950-t0W)*log(datos$Female[datos$Year==B])        
      Rsum2<-Rsum2 + ((datos$Year[datos$Year==B])-1950-t0W)
      Rsum3<- Rsum3 + ((datos$Year[datos$Year==B])-1950-t0W)^2
      sum4<- sum4 + ((datos$Year[datos$Year==B])-1950-t0W)^4
      Ssum1<-Ssum1 +  ((datos$Year[datos$Year==B])-1950-t0W)^2*log(datos$Female[datos$Year==B])        
      Ssum2<-Ssum2 + ((datos$Year[datos$Year==B])-1950-t0W)^2
      Ssum3<- Ssum3 + ((datos$Year[datos$Year==B])-1950-t0W)^3
      Ssum4<- Ssum4 + ((datos$Year[datos$Year==B])-1950-t0W)^4
      
      
      
     # Rsum1M<-Rsum1M +  ((datos$Year[datos$Year==B])-1950-t0M)*log(datos$Male[datos$Year==B])        
    #  Ssum1M<-Ssum1M +  ((datos$Year[datos$Year==B])-1950-t0M)^2*log(datos$Male[datos$Year==B])        
      
      Rsum1M<-Rsum1M +  ((datos$Year[datos$Year==B])-1950-t0M)*log(datos$Female[datos$Year==B])        
      Rsum2M<-Rsum2M + ((datos$Year[datos$Year==B])-1950-t0M)
      Rsum3M<- Rsum3M + ((datos$Year[datos$Year==B])-1950-t0M)^2
      sum4M<- sum4M + ((datos$Year[datos$Year==B])-1950-t0M)^4
      Ssum1M<-Ssum1M +  ((datos$Year[datos$Year==B])-1950-t0M)^2*log(datos$Female[datos$Year==B])        
      Ssum2M<-Ssum2M + ((datos$Year[datos$Year==B])-1950-t0M)^2
      Ssum3M<- Ssum3M + ((datos$Year[datos$Year==B])-1950-t0M)^3
      Ssum4M<- Ssum4M + ((datos$Year[datos$Year==B])-1950-t0M)^4
      
      
      
           }  ## fin for B
    T1F<-(Ssum1-log(datos$Female[datos$Year==1950])*Ssum2)*Rsum3/ (Rsum3*Ssum4-Ssum3^2)
    T2F<-(Rsum1-log(datos$Female[datos$Year==1950])*Rsum2)*Ssum3/ (Rsum3*Ssum4-Ssum3^2)  
    T1M<-(Ssum1M-log(datos$Male[datos$Year==1950])*Ssum2M)*Rsum3M/ (Rsum3M*Ssum4M-Ssum3M^2)
    T2M<-(Rsum1M-log(datos$Male[datos$Year==1950])*Rsum2M)*Ssum3M/ (Rsum3M*Ssum4M-Ssum3M^2)  
   alphaMuj1[A+1]<-T1F-T2F
   alphaHom1[A+1]<-T1M-T2M
   

   alphaMuj0[A+1]<- (Rsum1-log(datos$Female[datos$Year==1950])*Rsum2)/ Rsum3 -alphaMuj1[A+1]*Ssum3
   alphaHom0[A+1]<- (Rsum1M-log(datos$Male[datos$Year==1950])*Rsum2)/ Rsum3-alphaHom1[A+1]*Ssum3 
...
for(A in ages) {
  ...
  datos1$FemalePost<-log(datos1$Female)-log(datos1$Female[1])-alphaMuj0[A+1]*(datos1$Year-T0Wo[A+1])-alphaMuj1[A+1]*(datos1$Year-T0Wo[A+1])^2
  datos1$MalePost<-log(datos1$Male)-log(datos1$Male[1])-alphaHom0[A+1]*(datos1$Year-T0Me[A+1])-alphaHom1[A+1]*(datos1$Year-T0Me[A+1])^2
...
for(h in ages){
  H1<-HMujeres[[h+1,2]]
  H2<-HHombres[[h+1,2]]
  datos2$FemalePost[datos2$Age==ages[h+1]]<-datos2$FemalePost[datos2$Age==ages[h+1]]#*(55^H1)
  datos2$MalePost[datos2$Age==ages[h+1]]<-datos2$FemalePost[datos2$Age==ages[h+1]]#*(55^H2)
}

#x11(); par(las=1)
plot(HMujeres[,1],HMujeres[,2], type='l', col="blue",  ylim=c(0,1), main="Hurst estimation  for women by using 3 different R-libraries",xlab='Age', ylab='H')
  lines(HMujeres[,1],HMujeres[,3], col="red",lty=2)  
  lines(HMujeres[,1],HMujeres[,4], col="black",lty=3)  
  legend("bottomright",c("hurstexp","FDWhittle","Rovers"),lty=1:3,col=c("blue","red","black"),bty = "n")
  

  
#x11(); par(las=1)
plot(HHombres[,1],HHombres[,2], type='l', col="blue",  ylim=c(0,1), main="Hurst estimation for men by using 3 different R-libraries",xlab='Age', ylab='H')
  
  lines(HHombres[,1],HHombres[,3], col="red",lty=2)  
  lines(HHombres[,1],HHombres[,4], col="black",lty=3)  
  legend("bottomright",c("hurstexp","FDWhittle","Rovers"),lty=1:3,col=c("blue","red","black"),bty = "n")
...
# for(i in 1931:2004)
#  {  
#    htWomen[i-1949,2]<-hw0*exp(alphaMuj0[age+1]*i + DW[3,i-1949])
#    htMen[i-1949,2]<-hm0*exp(alphaHom0[age+1]*i + DM[1,i-1949])
#    htWomen[i-1949,1]<-i
#    htMen[i-1949,1]<-i
#   }
# 
 ...
 for(j in 2:cy-1)
  {
   if(j>1)
     {
     ht_hat[H,j]<- datos$Female[1]*exp(alphaMuj0[age+1]*(j-T0Wo[age+1]) +alphaMuj1[age+1]*(j-T0Wo[age+1])^2 + Yt_hat[j])
     ht_hatM[H,j]<- datos$Male[1]*exp(alphaHom0[age+1]*(j-T0Me[age+1]) + alphaHom1[age+1]*(j-T0Me[age+1])^2 +Yt_hatM[j])
     }
  }
...
#### arriba para guardas las 1000 simulaciones de las tasas ht_hat
### Women
...
### Women
plot(years[1:54],datos$Female[1:54],type="l", col="blue",xlab="Years",ylab ="Mortality rates",
     main=bquote("Mortality rates for women at age"~ .(age) ))
lines(years[1:54],ht_hat_mean[1:54], type="l", col="red")
lines(years[1:54],ht_hat_mean[1:54]-Desv_stan[1:54], type="l", col="green")
lines(years[1:54],ht_hat_mean[1:54]+Desv_stan[1:54], type="l", col="green")
lines(years[1:54],ht_hat_mean[1:54]-2*Desv_stan[1:54], type="l", col="brown")
lines(years[1:54],ht_hat_mean[1:54]+2*Desv_stan[1:54], type="l", col="brown")
#### men
plot(years[1:54],datos$Male[1:54],type="l", col="blue",xlab="Years",ylab ="Mortality rates",
     main=bquote("Mortality rates for men at age"~ .(age) ))
lines(years[1:54],ht_hat_meanM[1:54], type="l", col="red")
lines(years[1:54],ht_hat_meanM[1:54]-Desv_stanM[1:54], type="l", col="green")
lines(years[1:54],ht_hat_meanM[1:54]+Desv_stanM[1:54], type="l", col="green")
lines(years[1:54],ht_hat_meanM[1:54]-2*Desv_stanM[1:54], type="l", col="brown")
lines(years[1:54],ht_hat_meanM[1:54]+2*Desv_stanM[1:54], type="l", col="brown")
